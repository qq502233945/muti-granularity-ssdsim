...
int  main()
{
	...
	char* chCurPath = _getcwd(NULL, 0);              // 当前工作目录
...
{  
	...
	***********************************************************************************************************/
	...
	if (device == 1)
	{
		if (lsn != 123)//不同计算开始，需要判断上一个阶段是否执行完
		{
			if (lsn == 777)
			{
				stage = 0;
...
/**********************************************************************************************************************************************
*根据请求的大小返回对应的粒度域 (这个部分粒度选择需要考虑空间利用率、读取速度)
***********************************************************************************************************************************************/
struct request* request_granularity(struct ssd_info* ssd,struct request* new_request)
{
	...
	request_siez = new_request->size*512; //根据lsn数量计算请求大小，每个lsn大小为512B
...
struct ssd_info* buffer_management(struct ssd_info* ssd)
{
	...
	//这里假设lsn与不同粒度lpn的映射是连续的，可通过python或者预处理解决
	...
	lpn = (new_request->lsn / (ssd->parameter->granularity_size[0] / 512))-(new_request->lsn/ (ssd->parameter->granularity_size[0] / 512))%(new_request->granularity / ssd->parameter->granularity_size[0]); //lpn地址空间根据最小的粒度来划分，按通道粒度分配。
	//lpn = new_request->lsn / ssd->parameter->subpage_page;
	last_lpn = ((lsn+ new_request->size-1) / (ssd->parameter->granularity_size[0] / 512)) - ((lsn + new_request->size-1) / (ssd->parameter->granularity_size[0] / 512)) % (new_request->granularity / ssd->parameter->granularity_size[0]);//不同粒度逻辑页地址根据粒度算，逻辑页和物理页相同
	first_lpn = lpn;
		...
		while (lpn <= last_lpn)
		{
			...
			*************************************************************************************************/
			...
			while ((buffer_node != NULL) && (lsn < (lpn + 1) * (ssd->parameter->granularity_size[0] / 512)) && (lsn <= (new_request->lsn + new_request->size - 1)))
			{
				...
				if (flag == 1) //LRU算法
				{	//如果该buffer节点不在buffer的队首，需要将这个节点提到队首，实现了5LRU算法，这个是一个双向队列。		       		
					if (ssd->dram->buffer->buffer_head != buffer_node)
					{
						if (ssd->dram->buffer->buffer_tail == buffer_node)
						{
			...
			index = (lpn - first_lpn) / (32 / (ssd->parameter->granularity_size[0] / 512));//计算第多少个flag表示当前页
	...
	else if (new_request->operation == WRITE)
	{
		...
		while (lpn <= last_lpn)
		{
			...
			if (lpn == first_lpn)
			{
				offset1 = (new_request->granularity / 512) - ((lpn + (new_request->granularity / ssd->parameter->granularity_size[0])) * (ssd->parameter->granularity_size[0] / 512) - new_request->lsn); //根据写请求的粒度计算该粒度下的偏移量
				state = state & (0xffffffff << offset1); //起始偏移量
			}
			if (lpn == last_lpn)
			{
				...
				state = state & (~(0xffffffff << offset2));//结束偏移量
			}
	...
	if(req != NULL)
	{
		if(req->distri_flag == 0)
		{
			...
			{		
				...
				end = ((last_lsn/ (ssd->parameter->granularity_size[0] / 512)) + 1) * (ssd->parameter->granularity_size[0] / 512);//该页最后的lsn号
				i = (end - start)/32;	
...
unsigned int transfer_size(struct ssd_info *ssd,unsigned long need_distribute,unsigned int lpn,struct request *req)
{
	...
	first_lpn = (req->lsn / (ssd->parameter->granularity_size[0] / 512)) - (req->lsn / (ssd->parameter->granularity_size[0] / 512)) % (req->granularity / ssd->parameter->granularity_size[0]); //lpn地址空间根据最小的粒度来划分，按通道粒度分配。
	//lpn = new_request->lsn / ssd->parameter->subpage_page;
	//last_lpn = lpn + ((req->size -1 )* 512) / req->granularity;//不同粒度逻辑页地址根据粒度算，逻辑页和物理页相同
...
__int64 find_nearest_event(struct ssd_info *ssd) 
{
	 ...
	 *注意可能 都没有满足要求的time，这时time返回0x7fffffffffffffff 。
	*****************************************************************************************************/
					...
					for (k=0;k<ssd->parameter->die_chip;k++)
						...
						{  
							...
							{  
								...
								{  
									ssd->granularity_head[r].channel_head[i].chip_head[j].die_head[k].plane_head[l].blk_head[m].page_head[n].valid_state=0;        //表示某一页失效，同时标记valid和free状态都为0
									ssd->granularity_head[r].channel_head[i].chip_head[j].die_head[k].plane_head[l].blk_head[m].page_head[n].free_state=0;         //表示某一页失效，同时标记valid和free状态都为0
									ssd->granularity_head[r].channel_head[i].chip_head[j].die_head[k].plane_head[l].blk_head[m].page_head[n].lpn=0;  //把valid_state free_state lpn都置为0表示页失效，检测的时候三项都检测，单独lpn=0可以是有效页
...
struct ssd_info *no_buffer_distribute(struct ssd_info *ssd)
{
	...
	lsn=req->lsn;
	lpn = (req->lsn / (ssd->parameter->granularity_size[0] / 512)) - (req->lsn / (ssd->parameter->granularity_size[0] / 512)) % (req->granularity / ssd->parameter->granularity_size[0]); //lpn地址空间根据最小的粒度来划分，按通道粒度分配。
	//lpn = new_request->lsn / ssd->parameter->subpage_page;
	last_lpn = ((lsn + req->size - 1) / (ssd->parameter->granularity_size[0] / 512)) - ((lsn + req->size - 1) / (ssd->parameter->granularity_size[0] / 512)) % (req->granularity / ssd->parameter->granularity_size[0]);//不同粒度逻辑页地址根据粒度算，逻辑页和物理页相同
	first_lpn = lpn;
	...
	else if(req->operation==WRITE)
	{
		...
		if (balance_flag&&((req->size)>(2*req->granularity/512*ssd->parameter->granularity_channel[req_grannnum(ssd,req->granularity)])))//如果擦除不均衡则将大的请求中的部分按通道数量以及读写速度分配到更小粒度的闪存
		//if (balance_flag)
		{
			
			if (req->granularity == 16384)//如果是tlc且tlc磨损更严重则重分配，计算需要分配的lpn区间
			{
				ssd->count_valid0++;
				for (i = 0; i <= ssd->parameter->granularity_num; i++)
				{
		...
		while (lpn <= last_lpn)
		{
			if (balance_flag)//需要重映射
			{
				...
				if (ssd->parameter->granularity_size[temp] != 16384)//当前还没映射到tlc闪存
				{
					if (map_lpn[temp] > 0 )
					{
						remap_fla = 1;
			...
			if (lpn == first_lpn)
			{
				...
				offset1 = (req->granularity / 512) - ((lpn + (req->granularity / ssd->parameter->granularity_size[0])) * (ssd->parameter->granularity_size[0] / 512) - req->lsn); //根据写请求的粒度计算该粒度下的偏移量
				state = state & (0xffffffff << offset1); //起始偏移量
			}
			if (lpn == last_lpn)
			{
