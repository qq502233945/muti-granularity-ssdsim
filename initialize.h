...
struct ac_time_characteristics {
	long long  tPROG;     //program time
	long long tDBSY;     //bummy busy time for two-plane program
	long long tBERS;     //block erase time
	long long tCLS;      //CLE setup time
	long long tCLH;      //CLE hold time
	long long tCS;       //CE setup time
	long long tCH;       //CE hold time
	long long tWP;       //WE pulse width
	long long tALS;      //ALE setup time
	long long tALH;      //ALE hold time
	long long tDS;       //data setup time
	long long tDH;       //data hold time
	long long tWC;       //write cycle time
	long long tWH;       //WE high hold time
	long long tADL;      //address to data loading time
	long long tR;        //data transfer from cell to register
	long long tAR;       //ALE to RE delay
	long long tCLR;      //CLE to RE delay
	long long tRR;       //ready to RE low
	long long tRP;       //RE pulse width
	long long tWB;       //WE high to busy
	long long tRC;       //read cycle time
	long long tREA;      //RE access time
	long long tCEA;      //CE access time
	long long tRHZ;      //RE high to output hi-z
	long long tCHZ;      //CE high to output hi-z
	long long tRHOH;     //RE high to output hold
	long long tRLOH;     //RE low to output hold
	long long tCOH;      //CE high to output hold
	long long tREH;      //RE high to output time
	long long tIR;       //output hi-z to RE low
	long long tRHW;      //RE high to WE low
	long long tWHR;      //WE high to RE low
	long long tRST;      //device resetting time
}ac_timing;
...
struct ssd_info {
	...
	unsigned int program_count_level;
	unsigned int program_count_pre;
	unsigned int program_count_gc;
	unsigned int program_count_get;
	unsigned int count_valid;
	unsigned int count_valid0;
	unsigned int count_valid1;
	unsigned int count_valid2;
	unsigned int count_valid3;
	unsigned int count_valid4;
	...
	unsigned int last_lpn_granu[4];
	unsigned int lpn_granu[4];
	float earse_factor[4];
	float map_factor[4];
	float map_factor1[4];
	FILE* outputfile;
	FILE* tracefile;
	...
	struct granularity_info* granularity_head; //指向粒度结构体数组的首地址
	struct sub_request* subs_w_g_head[4]; //不同粒度的队列
	struct sub_request* subs_w_g_tail[4]; //不同粒度的请求队列
};

struct granularity_info                  //粒度域划分
{
	unsigned int progranm_count;
	unsigned int granularity_size;       //表示改粒度域的大小
	unsigned long erase_count;
	struct channel_info* channel_head;   //指向channel结构体的首地址
...
struct blk_info {
	...
	unsigned int size;				   //记录该块每页page的大小
...
typedef struct buffer_group {
	...
	unsigned int granulariy;
	unsigned int group;                 //the first data logic sector number of a group stored in buffer 
	unsigned long stored;                //indicate the sector is stored in buffer or not. 1 indicates the sector is stored and 0 indicate the sector isn't stored.EX.  00110011 indicates the first, second, fifth, sixth sector is stored in buffer.
...
struct request {
	...
	unsigned int granularity;			//请求分配的粒度域
	unsigned long* need_distr_flag;
...
struct sub_request {
	...
	unsigned long state;              //使用state的最高位表示该子请求是否是一对多映射关系中的一个，是的话，需要读到buffer中。1表示是一对多，0表示不用写到buffer
...
struct sub_request_two {
	struct sub_request* one;    
	struct sub_request* two;    
};
...
struct parameter_value {
	...
	unsigned int granularity_num;       //记录一个SSD中有多少个粒度域
	unsigned int granularity_channel[8]; //从小到大记录每个粒度域通道数量 
	unsigned int granularity_size[8]; //从小到大记录每个粒度域大小 
...
struct entry {
	...
	unsigned long state;                      //十六进制表示的话是0000-FFFF，每位表示相应的子页是否有效（页映射）。比如在这个页中，0，1号子页有效，2，3无效，这个应该是0x0003.
};


struct local {
	unsigned int granularity_num;
	 int channel;
	 int chip;
	 int die;
	 int plane;
